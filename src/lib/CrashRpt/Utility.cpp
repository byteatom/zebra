...
{  
	...
	int absPos = 0;
	if ((absPos = sFolderName.Find(_T(":\\"), 0)) >= 0 
		|| (absPos = sFolderName.Find(_T(":/"), 0)) >= 0)
		pos = absPos + 2;

    while (true)
    {
        int sepPos = sFolderName.Find('\\', pos);
        if (sepPos < 0)
        {
			sepPos = sFolderName.Find('/', pos);
			if (sepPos < 0)
				sIntermediateFolder = sFolderName;
			else
				sIntermediateFolder = sFolderName.Left(sepPos);
        }
        else
        {
            sIntermediateFolder = sFolderName.Left(sepPos);
