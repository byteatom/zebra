...
void MimeMessage::setFrom(EmailAddress* e)
{
	this->from = e;
}
...
void MimeMessage::setReplyTo(const QString & addr)
{
	replyTo = addr;
}
...
const EmailAddress & MimeMessage::getFrom() const
{
	return *from;
}
...
QString MimeMessage::toString()
{
	...
	if (from->getName() != "")
	{
		...
		case MimePart::Base64:
			mime += " =?utf-8?B?" + QByteArray().append(from->getName()).toBase64() + "?=";
			break;
		case MimePart::QuotedPrintable:
			mime += " =?utf-8?Q?" + QuotedPrintable::encode(QByteArray().append(from->getName())).replace(' ', "_").replace(':',"=3A") + "?=";
			break;
		default:
			mime += " " + from->getName();
		}
	}
	mime += " <" + from->getAddress() + ">\r\n";
	/* ---------------------------------- */
	...
	if (!replyTo.isEmpty())
		mime.append("Reply-To: ").append(replyTo).append("\r\n");
